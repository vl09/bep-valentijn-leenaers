...
It should be noted that many peaks remain unassigned, the scope floor sits above the datasheet limit, the strongest bursts approach $\pm 2\,\mathrm{g}$, and features near $\sim 2.2\,\mathrm{kHz}$ may partly reflect the sensor transfer function. These limits do not undo the cooler-on comb and axis-dependent peaks standing clear of the cooler-off floor, but they define what the next measurements should target.
