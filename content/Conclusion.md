...
SteeleLab's main experiments run on Bluefors dilution refrigerators precooled by pulse-tube cryocoolers[@steeleLabResearch]. Literature on such systems reports a narrow-band component near $1.4\,\mathrm{Hz}$ and its harmonics[@maisonobe2018], a different periodic drive from the $\approx 1\,\mathrm{Hz}$ GM comb measured on the DIY 4K fridge. The same ADXL354 chain on a Bluefors stage would characterise that precooler drive on the cryostat and allow comparison with spectra at lower mounting spots.
...
In conclusion, cryostat vibration limits how precisely phonon motion can be read out in microwave optomechanics, and the cooler cycle is a periodic part of that disturbance. This thesis showed that a calibrated ADXL354 chain can measure it on a GM-cooled final stage relative to a cooler-off baseline. What is recorded here is room-temperature commissioning data on one fridge at one mount point, but the same calibrated chain can now be applied at colder plate temperatures, on other cryostats, and on the cryogenic mass-spring platforms SteeleLab uses to shield sensitive devices. Tracing how cooler drive propagates along that path would show where isolation or mounting changes are needed. Reducing the drive at the lowest stage would lower the mechanical noise floor and improve phonon readout in future optomechanical experiments at SteeleLab.
