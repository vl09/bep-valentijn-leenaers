...
The DIY 4K fridge used in this thesis is cooled by a Gifford–McMahon (GM) cryocooler. In this cycle, a displacer moves helium gas between hot and cold volumes. The motion compresses and expands the gas in steps, transferring heat out of the cold stage. Each cycle therefore imposes a periodic mechanical disturbance on the fridge structure.

In the time domain the disturbances appear as a repeated pattern of accelerations rather than a smooth sinusoid. A fast displacement stroke followed by a return produces a characteristic tick-back-tick sequence in the acceleration trace, sometimes with a double peak within one half-cycle when the internal valve and displacer motion are offset in time. 

Literature on dry cryostats reports a narrow-band mechanical component near $1.4\,\mathrm{Hz}$ from cryocooler operation, together with low-frequency harmonics[@maisonobe2018]. Wilkinson[@wilkinson2025] describes a related periodic ticking from helium-pump strokes in a pulse-tube Bluefors system. The GM cooler in the present setup produces a fundamental near $1\,\mathrm{Hz}$. The absolute frequency depends on the cooler motor speed and duty cycle.

## The ADXL354 accelerometer
The measurement chain centres on the ADXL354; a low noise density, low 0 g offset drift, low power, 3-axis microelectromechanical systems (MEMS) accelerometer with selectable measurement ranges[@adxl354_datasheet]. The accelerometer is configured for the $\pm 2\,\mathrm{g}$ full-scale range in this thesis. Each analog output is ratiometric to the on-chip $1.8\,\mathrm{V}$ supply $\mathrm{V1P8ANA}$: the zero-$g$ bias is nominally $\mathrm{V1P8ANA}/2 = 0.9\,\mathrm{V}$, and the datasheet quotes a typical sensitivity of $400\,\mathrm{mV/g}$ at this range.

The sensor does not respond uniformly at all frequencies. Analog Devices publishes measured transfer functions for each axis, showing the magnitude of acceleration output in units of $g$ per $g$ of input as a function of frequency. These curves include the effect of the on-chip anti-aliasing filter and show an internal mechanical resonance near $2.5\,\mathrm{kHz}$ on the axes used here. Features in measured spectra near that frequency must therefore be interpreted with the sensor response in mind, not only as properties of the cryostat structure.

The datasheet also specifies a typical noise density of order $20\,\mu\mathrm{g}/\sqrt{\mathrm{Hz}}$ for the ADXL354 at relevant bandwidth settings. Measured baseline spectra on the bench can exceed this floor if the readout electronics add broadband noise. Such an offset does not prevent identification of coherent cryocooler lines well above the floor, but it limits how closely the absolute ASD can be compared to the datasheet specification without additional front-end amplification or shielding.

The $\pm 2\,\mathrm{g}$ range corresponds to $\pm 0.8\,\mathrm{V}$ of output swing about the zero-$g$ bias when the typical sensitivity applies, or $\pm 0.9\,\mathrm{V}$ when referenced to $\mathrm{V1P8ANA}/2$ on the oscilloscope channels. Time traces are checked against these limits to confirm that large cryocooler-induced accelerations remain within the linear range of the sensor.
