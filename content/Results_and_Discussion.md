...
That deviation is small compared with the $\pm 16\%$ production spread quoted in the datasheet. Additional contributors include the single benchtop measurement (no repeatability statistics), manual handling during the flip, scope digitisation, and unit-to-unit spread: the datasheet gives $z$-axis sensitivity repeatability of $0.3\%$ and a temperature coefficient of $\pm 0.01\%/^{\circ}\mathrm{C}$ over $-40\,^{\circ}\mathrm{C}$ to $+125\,^{\circ}\mathrm{C}$[@adxl354_datasheet], so a few degrees of laboratory temperature change cannot account for the $2\%$ offset from typical. Cross-axis coupling ($1\%$), nonlinearity ($0.1\%$ at $\pm 2\ \mathrm{g}$), and the use of one $z$-axis calibration for all three channels are further minor sources. 
