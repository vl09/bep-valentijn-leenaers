...
The baseline noise mountain from [](#results-baseline) is also visible in [](#fig-running-overlay-mid). In the grey cooler-off traces it appears as the same broad elevation centred near $25\ \mathrm{Hz}$. With the cooler on, the $1\ \mathrm{Hz}$ GM comb places a drive line at every integer frequency and superposes mechanical peaks on this cooler-independent bump. At crests near $26\ \mathrm{Hz}$, the coloured traces exceed the grey curves by factors of $\sim 3$–$11$ (e.g. $\sim 0.35$ vs $3.7\ \mathrm{mg}/\sqrt{\mathrm{Hz}}$ on $z$). Away from the integer-Hz lines, the cooler-on floor stays within roughly a factor of two of the cooler-off level in both this band and below $\sim 20\ \mathrm{Hz}$; the $15$–$18\times$ contrasts in [](#fig-running-overlay-low-z) therefore reflect the low-frequency comb peaks on $z$, not a higher broadband floor. The noise mountain persists as a recognizable cooler-off reference in the overlay, but is partially masked once GM harmonics at $24$–$26\ \mathrm{Hz}$ are added.
