...
Distinct, axis-dependent peaks persist well above $200\,\mathrm{Hz}$. The $x$-channel shows a peak near $677\,\mathrm{Hz}$ ($3.7\,\mathrm{mg}/\sqrt{\mathrm{Hz}}$) that is not prominent on $y$ or $z$. On $y$ and $z$, a strong peak appears near $488\,\mathrm{Hz}$ ($2.3$ and $10.9\,\mathrm{mg}/\sqrt{\mathrm{Hz}}$, respectively) that $x$ only shows at much lower strength. Near $1.3\,\mathrm{kHz}$, a broad mountain appears on all three channels, strongest on $z$ ($4.7\,\mathrm{mg}/\sqrt{\mathrm{Hz}}$ near $1290\,\mathrm{Hz}$), then $y$ ($3.2\,\mathrm{mg}/\sqrt{\mathrm{Hz}}$), with a weaker, slightly higher-frequency crest on $x$ ($\sim 2.5\,\mathrm{mg}/\sqrt{\mathrm{Hz}}$ near $1320\,\mathrm{Hz}$). That band lies well below the ADXL354 resonance near $2.5\,\mathrm{kHz}$ ([](#fig-adxl354-transfer)), so it is not read as a sensor artefact. 
...
All fridge-mounted spectra above were recorded with the final cold plate at room temperature, using the benchtop flip-test calibration from [](#results-static-flip-test). The cooler-on traces capture the GM mechanical drive on a warm stage, not on a plate held at $4\,\mathrm{K}$: each segment is only $\sim 10\,\mathrm{min}$ long and the plate warms again between sessions, so it does not reach cryogenic temperature during acquisition. Both the calibration and the cryostat recordings therefore fall within the ADXL354 operating temperature range of $-40$ to $+125\,^\circ\mathrm{C}$[@adxl354_datasheet].

The datasheet sensitivity temperature coefficient of $\pm 0.01\,\%$ per degree Celsius quoted in [](#results-static-flip-test) is specified over that same $-40$ to $+125\,^\circ\mathrm{C}$ band[@adxl354_datasheet]. It describes drift between ambient and industrial temperatures, not behaviour at cryogenic plate temperatures. Extrapolating it to $4\,\mathrm{K}$ would lie far outside the quoted range; whether the chip would operate or retain its calibrated sensitivity at such temperatures is not covered by the datasheet and remains unknown. For the present room-temperature data, sensitivity drift is therefore not a limiting factor: the plate never departs from the conditions under which $S = 392\,\mathrm{mV/g}$ was extracted.

The structural response is a separate concern. Resonance frequencies and damping of an extended copper plate depend on temperature through thermal expansion and elastic moduli, so the axis-dependent peaks above $\sim 70\,\mathrm{Hz}$ in [](#fig-running-asd-mid) and [](#fig-running-asd-full) need not occur at the same frequencies once the stage is cold. The $\approx 1\,\mathrm{Hz}$ GM drive period, set by displacer and valve timing, is expected to be far less sensitive to plate temperature than these structural modes. Cooler-off versus cooler-on contrasts at a given temperature remain informative for isolating the periodic drive, but extrapolating individual peak frequencies to cryogenic operation requires cold measurements. Repeating the spectroscopy at the nominal $4\,\mathrm{K}$ operating point would also require a cold calibration or another reference acceleration at that temperature, because datasheet guarantees on sensitivity and offset do not extend to cryogenic use.
